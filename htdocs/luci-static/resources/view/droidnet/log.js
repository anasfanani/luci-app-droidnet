...
return view.extend({
	...
	load: function () {
		...
		return Promise.all([
			poll.add(function () {
				fs.exec('/usr/bin/tail', ['/var/log/droidnet.log']).then(function (res) {
					var out = (res && res.stdout ? res.stdout.trim() : '');
					var err = (res && res.stderr ? res.stderr.trim() : '');

					if (err || !out) {
						ui.addNotification(_('Error: Read log file!'),
							E('p', _('Unable to read the interface info from /var/log/droidnet.log.') + (err ? ` (${err})` : '')),
							'danger'
						);
						notification = true;
						return;
					...
					syslog.textContent = data; // safer than innerHTML for logs
					notification = false;
